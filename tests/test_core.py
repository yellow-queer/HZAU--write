...
def test_endnote_xml_parsing():
    ...
    tmp_dir = tempfile.mkdtemp()
    try:
        xml_content = """<?xml version="1.0" encoding="UTF-8" ?><xml><records>
<record><rec-number>1</rec-number><ref-type name="Journal Article">17</ref-type><contributors><authors><author><style>He, Yushuang</style></author><author><style>Wang, Zhuo</style></author></authors></contributors><titles><title><style>Deep learning in crop disease image recognition</style></title><secondary-title><style>Journal of Agricultural Engineering</style></secondary-title></titles><periodical><full-title><style>Journal of Agricultural Engineering</style></full-title></periodical><dates><year><style>2023</style></year></dates><volume><style>44</style></volume><pages><style>148-155</style></pages><abstract><style>Crop disease recognition is essential for yield and quality.</style></abstract><keywords><keyword><style>deep learning</style></keyword><keyword><style>crop disease</style></keyword></keywords><electronic-resource-num><style>10.1234/test.2023</style></electronic-resource-num></record>
<record><rec-number>2</rec-number><ref-type name="Thesis">32</ref-type><contributors><authors><author><style>方钦</style></author></authors></contributors><titles><title><style>基于知识图谱的农业问答系统</style></title></titles><dates><year><style>2023</style></year></dates><publisher><style>浙江农林大学</style></publisher><abstract><style>农业信息化发展迅速</style><style>,</style><style>知识获取成为挑战。</style></abstract></record>
</records></xml>"""
        xml_file = Path(tmp_dir) / "test.xml"
        with open(xml_file, 'w', encoding='utf-8') as f:
            f.write(xml_content)

        lit_index = LiteratureIndex(literature_dir=tmp_dir)
        keys = lit_index.parse_endnote_xml(str(xml_file))

        assert len(keys) == 2, f"应解析2篇文献，实际：{len(keys)}"
        assert "ref_1" in keys, "缺少 ref_1"
        assert "ref_2" in keys, "缺少 ref_2"

        meta1 = lit_index.get_ref_metadata("ref_1")
        assert meta1 is not None, "ref_1 元数据为 None"
        assert meta1.get("ti") == "Deep learning in crop disease image recognition", f"标题错误：{meta1.get('ti')}"
        assert "He, Yushuang" in meta1.get("au", []), f"作者错误：{meta1.get('au')}"
        assert meta1.get("py") == "2023", f"年份错误：{meta1.get('py')}"
        assert meta1.get("jo") == "Journal of Agricultural Engineering", f"期刊错误：{meta1.get('jo')}"
        assert meta1.get("ty") == "Journal Article", f"类型错误：{meta1.get('ty')}"
        assert meta1.get("do") == "10.1234/test.2023", f"DOI错误：{meta1.get('do')}"

        meta2 = lit_index.get_ref_metadata("ref_2")
        assert meta2 is not None, "ref_2 元数据为 None"
        assert "方钦" in meta2.get("au", []), f"中文作者错误：{meta2.get('au')}"
        assert meta2.get("ti") == "基于知识图谱的农业问答系统", f"中文标题错误：{meta2.get('ti')}"
        assert "农业信息化发展迅速" in meta2.get("ab", ""), f"摘要拼接错误：{meta2.get('ab')}"
        assert meta2.get("pb") == "浙江农林大学", f"出版社错误：{meta2.get('pb')}"

        print("[OK] EndNote XML 文件解析测试通过")
    finally:
        shutil.rmtree(tmp_dir)
...
def test_file_structure():
    ...
    required_files = {
        ...
        "data/literature/柑橘.xml": "EndNote XML 文件",
        "README.md": "项目说明文档",
    }
    ...
    removed_files = [
        ...
        "data/literature/references.ris",
        "使用说明.md",
        "华中农业大学配置说明.md",
        "项目总结.md",
...
def test_state_definition():
    ...
    try:
        ...
        required_fields = [
            ...
            'research_context', 'citation_texts', 'word_limit', 'focus',
            'writing_notes'
        ]
...
def test_writing_directives():
    print("\n=== 测试写作指令注入 ===")

    try:
        from src.agents.writers import _build_writing_directives
    except ImportError as e:
        print(f"[SKIP] 无法导入 _build_writing_directives：{e}")
        return True

    state_full = {
        "word_limit": 3000,
        "focus": "重点分析实验结果",
        "writing_notes": ["避免复杂公式", "强调应用价值"]
    }
    result = _build_writing_directives(state_full)
    assert "3000" in result, f"字数限制未注入：{result}"
    assert "重点分析实验结果" in result, f"写作重心未注入：{result}"
    assert "避免复杂公式" in result, f"特殊要求未注入：{result}"
    assert "强调应用价值" in result, f"第二条特殊要求未注入：{result}"

    state_word_only = {"word_limit": 2500, "focus": "", "writing_notes": []}
    result = _build_writing_directives(state_word_only)
    assert "2500" in result, f"字数限制未注入：{result}"
    assert "写作重心" not in result, f"空重心不应注入：{result}"

    state_empty = {"word_limit": 0, "focus": "", "writing_notes": []}
    result = _build_writing_directives(state_empty)
    assert result == "", f"空指令应返回空字符串：{result}"
...
def run_all_tests():
    ...
    tests = [
        ...
        ("EndNote XML 文件解析", test_endnote_xml_parsing),
        ("写作指令注入", test_writing_directives),
    ]
